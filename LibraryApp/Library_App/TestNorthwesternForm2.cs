...
{
    public partial class TestNorthwesternForm2 : Form
    {
        ...
        public TestNorthwesternForm2()
        {
            ...
            lblAsk1.Paint += (sender, e) =>
            {
                ...
                Color borderColor = ColorTranslator.FromHtml("#1997d4"); // Цвет рамки
        ...
        private async void btnVar1_Click(object sender, EventArgs e)
        {
            SetButtonColor(btnVar1, Color.GreenYellow);
            // Задержка 1.0 секунды (1000 миллисекунд)
            await System.Threading.Tasks.Task.Delay(1000);

            using (var choiceForm = new ChoiceFormNW())
            {
                choiceForm.ShowDialog();

                if (choiceForm.Result == ChoiceFormNW.ChoiceResult.ReturnToDistrict)
                {
                    ReturnToDistrict();
                }
                else if (choiceForm.Result == ChoiceFormNW.ChoiceResult.ReturnToRussiaMap)
                {
                    ReturnToRussiaMap();
                }
            }
        ...
        private void ReturnToDistrict()
        {
            ...
            {
                if (!(form is NorthwesternMainForm || form is RegionMapForm || form is MainMenuForm || form is MapForm))
                {
                    form.Close();
                    GC.Collect();
                }
            ...
            var northwesternMainMenu = Application.OpenForms.OfType<NorthwesternMainForm>().FirstOrDefault();
            if (northwesternMainMenu == null)
            {
                northwesternMainMenu = new NorthwesternMainForm();
                northwesternMainMenu.Show();
            }
            else
            {
                northwesternMainMenu.BringToFront();
            }
    ...
    public class ChoiceFormNW : Form
    {
