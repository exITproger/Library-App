...
{
    public partial class TestFareastenForm2 : Form
    {
        ...
        public TestFareastenForm2()
        {
            ...
            lblAsk1.Paint += (sender, e) =>
            {
                ...
                Color borderColor = ColorTranslator.FromHtml("#74421f"); // Цвет рамки
        ...
        private async void btnVar2_Click(object sender, EventArgs e)
        {
            ...
            using (var choiceForm = new ChoiceFormFa())
            {
                choiceForm.ShowDialog();

                if (choiceForm.Result == ChoiceFormFa.ChoiceResult.ReturnToDistrict)
                {
                    ReturnToDistrict();
                }
                else if (choiceForm.Result == ChoiceFormFa.ChoiceResult.ReturnToRussiaMap)
                {
                    ReturnToRussiaMap();
                }
            }
        ...
        private void ReturnToDistrict()
        {
            ...
            {
                if (!(form is FareastenMainForm || form is RegionMapForm || form is MainMenuForm || form is MapForm))
                {
                    form.Close();
                    GC.Collect();
                }
            ...
            var fareastenMainMenu = Application.OpenForms.OfType<FareastenMainForm>().FirstOrDefault();
            if (fareastenMainMenu == null)
            {
                fareastenMainMenu = new FareastenMainForm();
                fareastenMainMenu.Show();
            }
            else
            {
                fareastenMainMenu.BringToFront();
            }
    ...
    public class ChoiceFormFa : Form
    {
