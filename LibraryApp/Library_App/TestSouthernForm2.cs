...
{
    public partial class TestSouthernForm2 : Form
    {
        ...
        private Color normalColor = ColorTranslator.FromHtml("#c70000");
        private Color hoverColor = Color.LightBlue;
        private PictureBox backgroundImage;
        public TestSouthernForm2()
        {
            ...
            lblAsk1.Paint += (sender, e) =>
            {
                ...
                Color borderColor = ColorTranslator.FromHtml("#000000"); // Цвет рамки
        ...
        private void ReturnToDistrict()
        {
            ...
            {
                if (!(form is SouthernMainForm || form is RegionMapForm || form is MainMenuForm || form is MapForm))
                {
                    form.Close();
                    GC.Collect();
                }
            ...
            var southernMainMenu = Application.OpenForms.OfType<SouthernMainForm>().FirstOrDefault();
            if (southernMainMenu == null)
            {
                southernMainMenu = new SouthernMainForm();
                southernMainMenu.Show();
            }
            else
            {
                southernMainMenu.BringToFront();
            }
        ...
        private async void btnVar3_Click(object sender, EventArgs e)
        {
            ...
            using (var choiceForm = new ChoiceFormSo())
            {
                choiceForm.ShowDialog();

                if (choiceForm.Result == ChoiceFormSo.ChoiceResult.ReturnToDistrict)
                {
                    ReturnToDistrict();
                }
                else if (choiceForm.Result == ChoiceFormSo.ChoiceResult.ReturnToRussiaMap)
                {
                    ReturnToRussiaMap();
                }
            }
    ...
    public class ChoiceFormSo : Form
    {
